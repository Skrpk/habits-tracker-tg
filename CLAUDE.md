...
14. **Analytics infers daily history — bounded by `lastCheckedDate`.** Daily habits store no per-day results; `computeCheckHistory` (`src/domain/utils/HabitAnalytics.ts`) reconstructs them: every day creation→today is `completed` **unless** explicitly in `skipped[]`/`dropped[]` **or after `lastCheckedDate`**. `lastCheckedDate` is the last day the user interacted (it advances on complete/skip/drop), so any day past it — a not-yet-checked today or a silently-missed/auto-paused run — renders as a **gap, not a phantom completion**. Misses are therefore always trailing, so the completed streak stays correct with no reset logic. **Caveat:** an *interior* silent miss (miss a day, then check later — which advances `lastCheckedDate` past it) leaves no stored trace and still infers as `completed`; fixing that needs recorded misses, not inference. Non-daily habits use the explicit `checked[]` array (no inference). Guard mirrored in `.cursorrules` §9.
